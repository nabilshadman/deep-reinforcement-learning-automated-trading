...
class DQNAgent(object):
  def __init__(self, state_size, action_size):
    ...
    self.epsilon = 0.0  # exploration rate --> set to 0.0 to avoid random action
  ...
  def act(self, state):
    ...
    return np.argmax(act_values[0]), act_values # returns action, values table
...
#   maybe_make_dir(rewards_folder)
    ...
    action, val = agent.act(state)
    print("Observation: ", state)
    print("Number of Actions: ", action_size)
    print("Action Chosen: ", action, " --> Decisions:", env.action_list[action])
    print("Max Value:", max(val[0]))
    print("Values: ", val)

    state_, reward, done, info = env.step(action)
    state_ = scaler.transform([state_])
    print("\nNext Observation:", state_)
    print("Reward:", reward)
    print("Done:", done)
    print("Info:", info)
