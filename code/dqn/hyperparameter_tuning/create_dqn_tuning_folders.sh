...
required_files=("dqn_trader.py" "equities_daily_close_2018_2023.csv" "clean.sh" "dqn_cpu.slurm" "dqn_gpu.slurm")
...
for device in cpu gpu; do
    for i in {1..27}; do
        folder_name="dqn_tuning_${device}_config${i}"
        mkdir -p "$folder_name"
        
        # Copy common files
        cp dqn_trader.py equities_daily_close_2018_2023.csv clean.sh "$folder_name/"
        
        # Copy and rename specific config file
        cp "config${i}.yaml" "$folder_name/config.yaml"
        
        # Copy corresponding slurm file
        cp "dqn_${device}.slurm" "$folder_name/"
